...
void loop() {
  ...
  delay(50); // Increased delay here

  if (distance > 35) {  // Condition For Absence Of Obstacle            
    Myservo.write(90);
    digitalWrite(MRb, HIGH);       // Move Forward
    digitalWrite(MRa, LOW);
    digitalWrite(MLb, HIGH);                                
    digitalWrite(MLa, LOW);                                                       
  } else if ((distance < 30) && (distance > 0)) {  // Condition For Presence Of Obstacle
    digitalWrite(MRb, LOW);     //Stop                
    digitalWrite(MRa, LOW);
    digitalWrite(MLb, LOW);                                
    digitalWrite(MLa, LOW);
    delay(230); // Increased delay here
    
    Myservo.write(0);
    delay(1000); // Increased delay here
    Myservo.write(180);
    delay(1000); // Increased delay here
    Myservo.write(90);
    delay(1000); // Increased delay here
    ...
    digitalWrite(MLb, LOW);                                
    digitalWrite(MLa, LOW);  
    delay(500); // Increased delay here  
